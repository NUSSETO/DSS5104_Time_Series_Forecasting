"""
End-to-end runner for Traffic dataset.

Usage:
    python runners/run_traffic.py                  # Full run
    python runners/run_traffic.py --smoke-test     # Quick validation
"""
...
def main(smoke_test: bool = False):
    ...
    n_series = 10 if smoke_test else cfg["n_series_sample"]
    max_steps = 10 if smoke_test else None
    n_windows = 1 if smoke_test else cfg["walk_forward_windows"]

    print(f"{'='*60}")
    print(f"Traffic — {'SMOKE TEST' if smoke_test else 'FULL RUN'}")
    print(f"  Series: {n_series}, Seeds: {len(seeds)}, Windows: {n_windows}")
    print(f"{'='*60}")

    # 1. Load data
    df_train, df_test = load_traffic(n_series=n_series)
    ...
    results_dir = RESULTS_DIR / "traffic"
    ...
    print(f"\n✓ Traffic complete. Results: {results_dir}")
    return results
